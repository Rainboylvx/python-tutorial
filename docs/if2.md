...
```python
# 获取用户输入，并将输入的字符串转换为整数类型
a = int(input("请输入数字"))

# 判断数字范围：如果小于 100 或者大于 999（即不是三位数）
if a < 100 or a > 999:
    print("no 1")  # 输入不符合要求（不是三位数），输出提示
else:
    # 如果是三位数，接下来分解出个位、十位和百位
    s1 = a % 10          # 取余数运算，获取【个位】数字
    s2 = a // 10 % 10    # 先整除10去掉个位，再取余数获取【十位】数字
    s3 = a // 100        # 整除100，直接获取【百位】数字
    
    # 核心判断：个位的立方 + 十位的立方 + 百位的立方 是否等于 原数字
    if s1*s1*s1 + s2*s2*s2 + s3*s3*s3 == a:
        print("yes")     # 满足条件，是水仙花数
    else:
        print("no 2")    # 是三位数，但不满足水仙花数条件
```
